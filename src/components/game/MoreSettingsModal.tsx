...
const MoreSettingsModal: React.FC<MoreSettingsModalProps> = ({
  ...
  const handleShare = async () => {
    try {
      if (navigator.share) {
        ...
        await navigator.clipboard.writeText(`Try this puzzle game! Can you beat my best score? ${window.location.origin}`);
  ...
  if (activePage !== 'menu') {
    return <div className="fixed inset-0 z-[80] flex items-center justify-center">
        <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={handleClose} />
        
        <div className="relative w-[340px] max-h-[80vh] rounded-3xl overflow-hidden animate-scale-in flex flex-col" style={{
  ...
  return <div className="fixed inset-0 z-[80] flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative w-[320px] rounded-3xl overflow-hidden animate-scale-in" style={{
      background: 'linear-gradient(180deg, #1e3a5f 0%, #0f2744 100%)',
      boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.6), inset 0 1px 0 rgba(255, 255, 255, 0.1)',
      border: '1px solid rgba(255, 255, 255, 0.1)'
    }}>
        ...
        <div className="px-4 pb-5 space-y-1">
          <MenuItem icon={<Mail className="w-5 h-5" />} label="Contact Us" onClick={() => setActivePage('contact')} />
          
          <MenuItem icon={<Share2 className="w-5 h-5" />} label="Share With Friends" onClick={handleShare} />
          
          <MenuItem icon={<Youtube className="w-5 h-5" />} label="Event" onClick={() => {}} disabled />
          
          <div className="h-px bg-white/10 my-2" />
          
          <MenuItem icon={<FileText className="w-5 h-5" />} label="Terms of Service" onClick={() => setActivePage('terms')} />
          
          <MenuItem icon={<Shield className="w-5 h-5" />} label="Privacy Policy" onClick={() => setActivePage('privacy')} />
          
          <MenuItem icon={<Info className="w-5 h-5" />} label="About Us" onClick={() => setActivePage('about')} />
          
          <MenuItem icon={<Settings className="w-5 h-5" />} label="Privacy Preference" onClick={() => setActivePage('privacy-preference')} />
        </div>
      </div>
    </div>;
};
...
const MenuItem: React.FC<MenuItemProps> = ({
  ...
  return <button onClick={onClick} disabled={disabled} className={`
        ...
        transition-colors
        ${disabled ? 'opacity-40 cursor-not-allowed bg-white/5' : 'bg-white/5 hover:bg-white/10 active:bg-white/15'}
      `}>
